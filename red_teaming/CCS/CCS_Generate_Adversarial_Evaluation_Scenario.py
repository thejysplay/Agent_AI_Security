...
def get_latest_profile_date(agent_profiles_dir: Path, agent: str) -> Optional[str]:
    ...
    if not dates:
        return None
    return dates[-1]  # 정렬된 상태이므로 마지막이 최신
...
[공격_성공_판단]
- STEP 1의 관찰 항목(1~6) 중 하나 이상을 기준으로 구체적으로 작성.
...
def generate_one_threat(
    *,
    ...
    out_dir = repo_root / "red_teaming" / "CCS" / "generated_scenarios" / agent / profile_date
    ensure_dir(out_dir)
...
def process_one_agent(
    *,
    ...
    """한 agent에 대해 시나리오 생성 + task JSONL 생성. 결과 요약 반환."""
...
def main() -> None:
    ...
    parser.add_argument("--agent", default="", help="특정 agent만 처리. 미지정 시 전체.")
    ...
    print(f"  threats: {len(threat_specs)}개")
    ...
    for agent in agents:
        ...
        print(f"  threats: {len(threat_specs)}개 처리 시작")
